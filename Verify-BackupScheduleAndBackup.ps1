<#
.DESCRIPTION
    This scipt highlights the missing backups after analyzing all backup policies schedules and verifies backup catalog with available list of backups. 
    ...
    4.  Download the script from script center. 
            wget https://github.com/anoobbacker/storsimpledevicemgmttools/raw/master/Verify-BackupScheduleAndBackup.ps1 -Out Verify-BackupScheduleAndBackup.ps1
            .\Verify-BackupScheduleAndBackup.ps1 -SubscriptionId [subid] -TenantId [tenantid] -ResourceGroupName [resource group] -ManagerName [device manager] -DeviceName [device name] -AuthNType [Type of auth] -AADAppId [AAD app Id] -AADAppAuthNKey [AAD App Auth Key]
