<#
.DESCRIPTION
    This script clones a StorSimple 8000 series volume.
...
.PARAMS 
    ...
    TargetDeviceName: Input the name of the StorSimple device on which to clone the volume.
...
try {
    ...
    $VolumesCount = (($SelectedBackup.Elements.VolumeName | select -Unique) | Measure-Object).Count

    If ($VolumesCount -gt 1) {
        $choiceIndex = 0
        $options = @()
        foreach($item in ($SelectedBackup.Elements.VolumeName | Sort-Object)) {
            $option = [System.Management.Automation.Host.ChoiceDescription]::new("&$choiceIndex - $($item.ToString())")
            $option.HelpMessage = $item.ToString()
            $options += $option
            $choiceIndex++
        ...
        $Info = "`nChoose below Volume"
        $opt = $host.UI.PromptForChoice($null, $Info, $Options, $defaultChoice)
        
        $SelectedBackupElement = ($SelectedBackup.Elements | Sort-Object VolumeName)[$opt]
        
    }
    else {
        $SelectedBackupElement = ($SelectedBackup.Elements | select)[$defaultChoice]
    }
    ...
    if ($TargetDevice -eq $null) {
        Write-Error "The entity Devices ($TargetDeviceName) was not found"
        break
    }
        
    $CloneRequest = New-Object Microsoft.Azure.Management.StorSimple8000Series.Models.CloneRequest
    $CloneRequest.TargetDeviceId = $TargetDevice.Id
    $CloneRequest.TargetVolumeName = $SelectedBackupElement.VolumeName + '_' + (Get-Date -Format ffff)
    $CloneRequest.TargetAccessControlRecordIds = $AccessControlRecordIds
    $CloneRequest.BackupElement = $SelectedBackupElement

    # Initiate clone the volume
    [Microsoft.Azure.Management.StorSimple8000Series.BackupsOperationsExtensions]::BeginClone($StorSimpleClient.Backups, $DeviceName, $SelectedBackup.Name, $SelectedBackupElement.ElementName, $CloneRequest, $ResourceGroupName, $ManagerName)
...
PrettyWriter "`nThe entity Clone volume ($($CloneRequest.TargetVolumeName)) successfully started the job.`n"
