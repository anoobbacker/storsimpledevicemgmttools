<#
.DESCRIPTION
    This script deletes a StorSimple 8000 series volume.
    ...
    4.  Download the script from script center. 
            wget https://github.com/anoobbacker/storsimpledevicemgmttools/blob/master/Delete-Volume.ps1 -Out Delete-Volume.ps1
            .\Delete-Volume.ps1 -SubscriptionId [subid] -TenantId [tenantid] -ResourceGroupName [resource group] -ManagerName [device manager] -DeviceName [device name] -VolumeContainerName [volume container name] -VolumeName [volume name] -AuthNType [Type of auth] -AADAppId [AAD app Id] -AADAppAuthNKey [AAD App Auth Key]
     ----------------------------
.PARAMS 
    ...
    DeviceName: Input the name of the StorSimple device on which to delete volume.
    ResourceGroupName: Input the name of the resource group.
    ManagerName: Input the name of the StorSimple device manager.
    VolumeContainerName: Input the name of the volume container on which to delete volume.
    VolumeName: Input the name of an existing volume.
...
(
    ...
    [parameter(Mandatory = $true, HelpMessage = "Input the name of the StorSimple device on which to delete volume.")]
    [String]
    $DeviceName,

    [parameter(Mandatory = $true, HelpMessage = "Input the name of the volume container on which to delete volume.")]
    [String]
    $VolumeContainerName,

    [parameter(Mandatory = $true, HelpMessage = "Input the name of an existing volume.")]
    [String]
    $VolumeName,
...
try {
    $list = [Microsoft.Azure.Management.StorSimple8000Series.VolumesOperationsExtensions]::ListByDevice($StorSimpleClient.Volumes, $DeviceName, $ResourceGroupName, $ManagerName)

    # Get Volume data
    $VolumeProperties = [Microsoft.Azure.Management.StorSimple8000Series.VolumesOperationsExtensions]::Get($StorSimpleClient.Volumes, $DeviceName, $VolumeContainerName, $VolumeName, $ResourceGroupName, $ManagerName)

    # Set VolumeStatus to Offline
    $VolumeProperties.VolumeStatus = [Microsoft.Azure.Management.StorSimple8000Series.Models.VolumeStatus]::Offline
    ...
    If ($Volume.VolumeStatus -eq [Microsoft.Azure.Management.StorSimple8000Series.Models.VolumeStatus]::Offline) {
        [Microsoft.Azure.Management.StorSimple8000Series.VolumesOperationsExtensions]::BeginDelete($StorSimpleClient.Volumes, $DeviceName, $VolumeContainerName, $VolumeName, $ResourceGroupName, $ManagerName)
    }
    else {
        Write-Error "`nThe entity Volume ($VolumeName) deletion failed.`n"
    }
}
catch {
    # Print error details
...
PrettyWriter "`nThe entity Volume ($VolumeName) successfully deleted.`n"
