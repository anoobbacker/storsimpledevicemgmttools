<#
.DESCRIPTION
    This script deletes a StorSimple 8000 series volume container.
    ...
    4.  Download the script from script center. 
            wget https://github.com/anoobbacker/storsimpledevicemgmttools/blob/master/Delete-VolumeContainer.ps1 -Out Delete-VolumeContainer.ps1
            .\Delete-VolumeContainer.ps1 -SubscriptionId [subid] -TenantId [tenantid] -ResourceGroupName [resource group] -ManagerName [device manager] -DeviceName [device name] -VolumeName [volume name] -AuthNType [Type of auth] -AADAppId [AAD app Id] -AADAppAuthNKey [AAD App Auth Key]
     ----------------------------
.PARAMS 
    ...
    DeviceName: Input the name of the StorSimple device on which to delete volume container.
    ResourceGroupName: Input the name of the resource group.
    ManagerName: Input the name of the StorSimple device manager.
    VolumeContainerName: Input the name of an existing volume container.
...
(
    ...
    [parameter(Mandatory = $true, HelpMessage = "Input the name of the StorSimple device on which to delete volume container.")]
    [String]
    $DeviceName,

    [parameter(Mandatory = $true, HelpMessage = "Input the name of an existing volume container.")]
    [String]
    $VolumeContainerName,
...
try {
    [Microsoft.Azure.Management.StorSimple8000Series.VolumeContainersOperationsExtensions]::BeginDelete($StorSimpleClient.VolumeContainers, $DeviceName, $VolumeContainerName, $ResourceGroupName, $ManagerName)
}
catch {
    # Print error details
...
PrettyWriter "`nThe entity Volume container ($VolumeContainerName) successfully deleted.`n"
